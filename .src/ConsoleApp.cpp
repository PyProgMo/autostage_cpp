...
std::string formatVelocityTestLine(double velocityNmPerS,
                                   double targetXNm,
...
void runVelocityTest(PIStageProxy& stage, double velocityNmPerS, double xDistanceNm) {
    ...
    while (true) {
        ...
        const std::string line = formatVelocityTestLine(
            velocityNmPerS,
            targetX,
            pos,
            deltaNm,
        ...
        // here the spectrometer could be triggered

        if (boundaryHit) {
            AppLogger::instance().error("velocitytest: boundary exceeded, halting stage");
            stage.halt();
            throw std::runtime_error("velocitytest aborted: stage left safe bounds");
        }

        if ((movingPositive && pos[0] >= targetX) || (!movingPositive && pos[0] <= targetX)) {
            AppLogger::instance().info("velocitytest: target reached");
            break;
        }
            ...
            if (target == "stage") {
                if (action == "connect") {
                    ...
                    if (iss >> velocityNmPerS >> xDistanceNm) {
                        runVelocityTest(*stage, velocityNmPerS, xDistanceNm);
